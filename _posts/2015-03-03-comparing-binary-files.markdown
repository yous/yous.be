...
As using [Git](https://git-scm.com/), looking changes of files with `git diff` is common. Always we can check the status of files with `git status`, including whether there is a new file that isn't added to index, existence of changed files.
...
You can use [`cmp`](https://en.wikipedia.org/wiki/Cmp_(Unix)) which compares two files byte by byte.
